...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.0) -> int:
        ...
        with torch.no_grad():
            ...
            greedy_action = qa_values.argmax(dim=1)       

            probs = torch.full(
                (1, self.num_actions),
                0.0,
                device=qa_values.device,
            )
            ...
            for i in range(self.num_actions):
                if i == greedy_action:
                    ...
                    probs[0, i] += epsilon / (self.num_actions - 1)
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            batch_indices = torch.arange(batch_size, device=next_qa_values.device)
            next_q_values = next_qa_values[batch_indices, next_action]
        ...
        q_values = qa_values[batch_indices, action]
