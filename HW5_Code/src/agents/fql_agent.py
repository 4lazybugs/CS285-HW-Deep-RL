...
class FQLAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray):
        ...
        with torch.no_grad():
            ...
            action = self.onestep_actor(observation, noise).clamp(-1, 1)
    ...
    def get_bc_action(self, observation: torch.Tensor, noise: torch.Tensor):
        ...
        for i in range(self.flow_steps):
            times = torch.full_like(action[..., :1], i / self.flow_steps)
            action = action + self.bc_actor(observation, action, times) / self.flow_steps
    ...
    def update_q(
        ...
        with torch.no_grad():
            ...
            next_actions = self.onestep_actor(next_observations, noise).clamp(-1, 1)
            next_q = self.target_critic(next_observations, next_actions).mean(dim=0)
            target_q = rewards + self.discount * (1 - dones.to(rewards.dtype)) * next_q
    ...
    def update_bc_actor(
        ...
        velocities = self.bc_actor(observations, interpolated_actions, times)
        loss = (velocities - (actions - noise)).pow(2).mean()
    ...
    def update_onestep_actor(
        ...
        actor_actions = self.onestep_actor(observations, noise)
        distill_loss = self.alpha * (actor_actions - bc_actions).pow(2).mean()

        # Hint: *Do* clip the one-step actor actions when feeding them to the critic
        q_loss = -self.critic(observations, actor_actions.clamp(-1, 1)).mean()

        # Total loss.
        loss = distill_loss + q_loss

        # Additional metrics for logging.
        mse = (actor_actions.clamp(-1, 1) - actions).pow(2).mean()
    ...
    def update_target_critic(self) -> None:
        ...
        with torch.no_grad():
            for target_param, critic_param in zip(
                ...
                target_param.lerp_(critic_param, self.target_update_rate)
