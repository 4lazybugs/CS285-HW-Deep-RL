...
class SACBCAgent(nn.Module):
    ...
    def update_q(
        ...
        with torch.no_grad():
            ...
            target_value = rewards + (1 - dones) * self.discount * target_q
        loss = (q - target_value).pow(2).mean()
    ...
    def update_actor(
        ...
        action_pi = actor_dists.rsample()
        q = self.critic(observations, action_pi)
        q_loss = -q.mean()

        mses = (actions - action_pi).pow(2).mean(dim=-1)
        bc_loss = self.alpha * mses.mean()

        log_probs = actor_dists.log_prob(action_pi)
