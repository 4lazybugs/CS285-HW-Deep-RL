...
class IQLAgent(nn.Module):
    ...
    def iql_expectile_loss(
        ...
        weight = torch.where(
            adv > 0,
            torch.as_tensor(expectile, device=adv.device, dtype=adv.dtype),
            torch.as_tensor(1.0 - expectile, device=adv.device, dtype=adv.dtype),
        )
    ...
    def update_v(
        ...
        loss = self.iql_expectile_loss(target_q - v, self.expectile)
    ...
    def update_actor(
        ...
        with torch.no_grad():
            ...
            advantage = q - self.value(observations)
            weights = torch.exp(self.alpha * advantage).clamp(max=100.0)
    ...
    def update_target_critic(self) -> None:
        ...
        with torch.no_grad():
            for target_param, critic_param in zip(
                ...
                target_param.copy_(
                    (1 - self.target_update_rate) * target_param
                    + self.target_update_rate * critic_param
                )
