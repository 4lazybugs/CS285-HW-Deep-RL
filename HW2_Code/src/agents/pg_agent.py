...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        return_from_rewards, gamma_n = 0, 1
        for reward in rewards:
            return_from_rewards += gamma_n*reward
            gamma_n = self.gamma*gamma_n

        return [return_from_rewards] * len(rewards)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t in range(len(rewards)):
            return_list.append(
                self._discounted_return(rewards[t:])[0]
            )
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [
                np.array(self._discounted_return(reward))
                for reward in rewards
            ]
        else:
            ...
            q_values = [
                np.array(self._discounted_reward_to_go(reward))
                for reward in rewards
            ]
